...
-- Creates missing tables only, so existing data stays untouched.
-- Use this for daily startup when you want to keep local data.
...
-- Add default categories if they are missing.
...
-- Seed default accounts if they are missing.
-- INSERT IGNORE keeps this safe to run more than once.

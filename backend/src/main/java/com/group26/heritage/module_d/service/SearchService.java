...
public class SearchService {
    ...
    public List<CategoryOptionDto> listCategories() {
        return categoryRepository.findAllByStatusOrderByNameAsc(CategoryStatus.ACTIVE).stream()
                .map(c -> new CategoryOptionDto(c.getId(), c.getName(), c.getStatus().name()))
                .toList();
    }

    public List<TagOptionDto> listTags() {
        return tagRepository.findAllByIsDeletedFalseOrderByNameAsc().stream()
    ...
    private List<Long> normalizeTagIds(List<Long> tagIds) {
        ...
        Set<Long> activeTagIds = tagRepository.findAllById(normalized).stream()

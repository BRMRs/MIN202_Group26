...
function ImageGallery({ mediaFiles }) {
  ...
  const next = useCallback(
    () => setCurrentIndex((i) => (i + 1) % mediaFiles.length),
    [mediaFiles.length],
  );
...
function ResourceDetailPage() {
  ...
  const mediaFiles = useMemo(() => {
    if (!resource) return [];
    if (resource.media?.length) return resource.media;
    if (resource.fileUrl) {
      return [{ id: 0, fileUrl: resource.fileUrl, fileName: null }];
    }
    return [];
  }, [resource]);
    ...
    <div className="d-page" style={{ maxWidth: '1050px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '32px' }}>
        ...
        <div>
          ...
          <p style={{ whiteSpace: 'pre-wrap', color: '#333', lineHeight: 1.65, margin: '12px 0' }}>
          ...
          <div style={{ background: '#f8f9fa', padding: '14px 16px', borderRadius: '8px', fontSize: '0.88em', lineHeight: 2 }}>
            ...
            <div>
              ...
              {resource.externalLink ? (
                <a href={resource.externalLink} target="_blank" rel="noopener noreferrer" style={{ color: '#007bff' }}>
                  View source ↗
                </a>
              ) : (
                'Not provided'
          ...
          {comments.length > 0 ? (
            comments.map((c) => (
              <div key={c.id} style={{ display: 'flex', gap: '12px' }}>
                ...
                <div>
                  <div style={{ fontWeight: 'bold', fontSize: '0.88em' }}>
